from . import cn006_proyecto_clasificacion
from . import cn006_proyecto_grado_complejidad
from . import cn006_proyecto_nivel_importancia
from . import cn006_proyecto_nivel_urgencia
from . import cn006_proyecto_tamano
from . import cn006_proyecto_tarea_grado_avance
from . import cn006_proyecto_tarea_tipificacion
from . import cn006_proyecto_tarea_tipo_soporte

from . import project_project_stage
from . import project_project
from . import project_task_type
from . import project_task

