...
def agregar_partes_horas_por_lotes(p_proyectos,p_tools: cCN006_globales ):
    ...
    if p_tools.g_debug:
        ...
        print(f"Por procesar {len(p_proyectos)} registros\n") 
...
def main(p_ambiente, p_debug) :
    ...
    try:
        ...
        # Obtener proyectos que se procesarán
        proyectos = []
        proyectos = obtener_proyectos(Tools)

        if Tools.g_debug:
            if len(proyectos ) > 0:
                print(f"Se ecnontraron {len(proyectos)} registros")
            else:
                print (f"No hay registros para proyectos cn006")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pDebug', type=cCN006_globales.str2bool, nargs='?', const=True, default=False, help="True - Despliega mensajes de seguimiento | False - para correr en producción")
