...
"""
    v040 - Se agregan los campos de partes de horas (fecha creación, fecha aplicación y horas )
"""
...
"""  ******* EXPLICACIÓN DEL PROCESO
    ...
    4. En la estructura (proyectos_info_total) están todos los datos de proyectos, partes horas y tareas.
    5. El proceso (seleccionar_campos_finales) es el que define los campos a mostrar en el archivo Excel.
************************************************************************  """
#endregion
...
def obtener_partes_horas(p_proyectos, p_tools: cCN006_globales):
    ...
    for detalle in detalles_partes_horas:
            # msj_debug(f"Antes del cálculo de horas: unit_amount: ({detalle.get('unit_amount')})", p_tools)
            detalle['unit_amount_60'] = detalle.get('unit_amount', 0) / 24
            # msj_debug(f"Después del cálculo de horas: unit_amount_60: ({detalle.get('unit_amount_60')})", p_tools)
...
def seleccionar_campos_finales(p_proyectos_info_total, p_tools: cCN006_globales):
    ...
    campos_seleccionados = [
        ...
        {'nombre_actual': 'ph_project_id', 'nombre_nuevo': 'ph ID Proyecto'},
        {'nombre_actual': 'ph_id', 'nombre_nuevo': 'ph ID'},
        {'nombre_actual': 'ph_name', 'nombre_nuevo': 'ph Titulo'},
        {'nombre_actual': 'ph_date', 'nombre_nuevo': 'ph Fecha'},
        {'nombre_actual': 'ph_unit_amount', 'nombre_nuevo': 'ph Horas Decimal'},
        {'nombre_actual': 'ph_unit_amount_60', 'nombre_nuevo': 'ph Horas'},
        {'nombre_actual': 'ph_create_date', 'nombre_nuevo': 'ph Fecha Creacion'},
    ]
    ...
    for proyecto in p_proyectos_info_total:
        ...
        for campo in campos_seleccionados:
            ...
            if nombre_actual in ['ph_unit_amount', 'ph_unit_amount_60']:
                p_tools.msj_debug(f"nombre_actual: ({nombre_actual})\nph_unit_amount    ({proyecto.get('ph_unit_amount', -5)})\nph_unit_amount_60 ({proyecto.get('ph_unit_amount_60', -10)})")
            ...
            if (
                ( ("cn006_fecha" in nombre_actual) or (nombre_actual in ['ph_date', 'ph_create_date']))
                and isinstance(valor, datetime)
                ):
        ...
        msj_debug(f"Proyecto campos seleccionados\n  \n\n{nuevo_registro}\n\n", p_tools)
...
def main(p_ambiente, p_debug) :
    ...
    ****************************************************************************************  """
    ...
    try:
        ...
        msj_debug(f"080 Dato muestra \nph_unit_amount ({detalles_partes_horas[0].get('ph_unit_amount', 'N/A')})\nph_unit_amount_60 ({detalles_partes_horas[0].get('ph_unit_amount_60', 'N/A')})", tools)
        ...
        msj_debug(f"120 Dato muestra \nph_unit_amount ({proyectos_info_total[0].get('ph_unit_amount', 'N/A')})\nph_unit_amount_60 ({proyectos_info_total[0].get('ph_unit_amount_60', 'N/A')})", tools)
