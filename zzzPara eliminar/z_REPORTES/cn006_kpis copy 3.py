...
def agregar_partes_horas_por_lotes(p_proyectos,p_tools: cCN006_globales ):


    msj_debug(f"\n******   Iniciando método agregar_partes_horas_por_lotes  ({datetime.now().strftime("%Y-%m-%d %H:%M:%S")})\n******   Por procesar {len(p_proyectos)} registros\n", p_tools)


    proyectos_timesheets = []
    all_timesheet_ids = []

    # Recopilar todos los IDs de partes de horas de los proyectos 
    msj_debug("Recopilar IDS de partes de horas de los proyectos", p_tools)
    for proyecto in p_proyectos:
        timesheet_ids = proyecto.get('timesheet_ids', [])
        all_timesheet_ids.extend(timesheet_ids)
    
    msj_debug(f"Total de")


    # Obtener detalles de partes de horas en bloques
    msj_debug("Obtener detalles de partes de horas en bloques", p_tools)
    timesheets = timesheets_bloque(p_tools, all_timesheet_ids)

    # Crear un diccionario para mapear timesheet_id a sus detalles
    msj_debug("Crear un diccionario para mapear timesheet_id a sus detalles", p_tools)
    ...
    for proeycto in p_proyectos:
        ...
        if not timesheet_ids:
            ...
            proyecto['timesheet_user_id'] = ""
            proyecto['timesheet_user_name'] = ""
            proyecto['tiempo_abierto'] = 0  # Ajustar el campo tiempo_abierto
            proyectos_timesheets.append(ticket)
        else:
            ...
            for i, timesheet_id in enumerate(timesheet_ids):
                ...
                if timesheet:
                    ...
                    proyectos_timesheets.append(ticket_with_timesheet)

        if p_tools.g_debug:
            msj_debug(f"Finalizó agregar_partes_horas_por_lotes  ({datetime.now().strftime("%Y-%m-%d %H:%M:%S")})\n\n\nEstos son los datos con sus partes de horas", p_tools)
            tickets_str = "\n".join([f"*** {ticket} ***" for ticket in proyectos_timesheets])
            msj_debug(tickets_str, p_tools)
            msj_debug(f"\nFin de datos con partes de horas\n", p_tools)

    return proyectos_timesheets



def timesheets_bloque(p_tools, all_timesheet_ids,  chunk_size=300):
    """Obtener detalles de partes de horas de Odoo en bloques para evitar timeout."""
    
    # Definir la conexión XML-RPC
    models = xmlrpc.client.ServerProxy(f"{p_tools.cnx_url}/xmlrpc/2/object")

    db = p_tools.cnx_db
    uid = p_tools.cnx_uid
    password = p_tools.cnx_password
...
def main(p_ambiente, p_debug) :
    ...
    try:
        ...
        for proyecto in proyectos:
            print(f"ID: {proyecto['id']}, Nombre: {proyecto['name']}, Responsable: {proyecto.get('user_id', 'N/A')}, Inicio: {proyecto.get('date_start', 'N/A')}")
        #endregion Obtener proyectos que se procesarán



        # Agregar partes de horas a los proyectos
        msj_debug(f"070 - Llamando agregar_partes_horas_por_lotes", tools)
        tickets_proyectos = []
        tickets_proyectos = agregar_partes_horas_por_lotes(proyectos,tools)
        msj_debug(f"080 - Regresando agregar_partes_horas_por_lotes", tools)

        Tools.jalj = "Hola mundo"
